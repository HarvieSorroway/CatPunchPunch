...
{
    public class OpSliderFloat : UIconfig, SelectableUIelement
	{
        /// <summary>
        ...
        /// <param name="length">Length of this slider will be this in pxl(width is 30 pxl)</param>
        ...
        public OpSliderFloat(Vector2 pos, string key, Vector2 range, int length, bool vertical = false, float defaultValue = 0) : base(pos, new Vector2(), key, defaultValue.ToString())
        {
            this.vertical = vertical;
            this.description = this.vertical ? "Hold your mouse button and Drag up/down to adjust value" : "Hold your mouse button and Drag left/right to adjust value";
            this.min = range.x; this.max = range.y;
            float r = this.max - this.min + 1;
            this.wheelTick = r > 5 ? Math.Max((r / 12f), 4) : 1;
            float l = Mathf.Max((float)(r - 1), (float)length);
            this.mul = l / (float)(r - 1);
            this._size = this.vertical ? new Vector2(30f, l) : new Vector2(l, 30f);
            this._value = Mathf.Clamp(defaultValue, min, max).ToString();
            this.defaultValue = this.value;
            if (!_init) { return; }
            Initialize();
        }

        #pragma warning disable CS0184 // 命名样式
        private bool subtleSlider => this is OpSliderSubtle;
        #pragma warning restore CS0184 // 命名样式
        ...
        public override void GrafUpdate(float dt)
        {
            ...
            if (!vertical)
            {
                if (!subtleSlider)
                {
                    float p = this.mul * (float)(this.valueFloat - this.min);
                ...
                {
                    float t = (this.valueFloat - this.min) * this.mul;
        ...
        public override void Update(float dt)
        {
            ...
            if (this.held)
            {
                if (Input.GetMouseButton(0))
                {
                    ...
                    this.valueFloat = Mathf.Clamp(t / this.mul + this.min, this.min, this.max);
                }
                else
                {
                    this.held = false;
            ...
            else if (!this.held && this.menu.manager.menuesMouseMode && this.MouseOver)
            {
                ...
                {
                    float num = valueFloat - Mathf.Sign(this.menu.mouseScrollWheelMovement) * this.wheelTick;
